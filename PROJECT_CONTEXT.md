...
- Flat Timer tiles reserve a separate title row, keep the time on the left, and keep adjustment/Stop/Resume/Reset controls on the right. Empty Reminders require an explicit “mark today clear” action, and completed reminder text never receives an opaque white pressed field.
