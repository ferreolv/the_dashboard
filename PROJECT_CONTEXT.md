...
- In the Library, tiles already on Today use a small half-outside corner cross instead of an in-tile “Remove from Today” footer. That cross is a direct, absolutely positioned tile child—never part of the title-control wrapper—so its centre sits exactly on the top-right corner without shifting the header. Compact Birthdays shows only today’s cake/name or the next birthday; its complete list and import controls live in a separate overlay.
